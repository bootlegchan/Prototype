...
func _recursive_load_definitions(path: String) -> void:
	...
	while item_name != "":
		...
		if dir.current_is_dir():
			...
			# --- THIS IS THE FIX ---
			# Always replace the original, constant base path from Config.
...
func _add_collision_shape_to_entity(physics_body: Node3D, shape_type: String) -> void:
	...
	match shape_type.to_lower():
		"box": shape_resource = BoxShape3D.new()
		"sphere": shape_resource = SphereShape3D.new()
		"cylinder": shape_resource = CylinderShape3D.new()
		"capsule": shape_resource = CapsuleShape3D.new()
		_:
