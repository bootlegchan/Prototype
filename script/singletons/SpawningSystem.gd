...
func _ready() -> void:
	Debug.post("_ready called. Loading spawn lists.", "SpawningSystem")
	_load_all_spawn_lists()
	Debug.post("Loaded %s spawn lists." % _spawn_lists.size(), "SpawningSystem")
...
func _recursive_load(path: String) -> void:
	Debug.post("_recursive_load called for path '%s'." % path, "SpawningSystem")
	...
	while item_name != "":
		...
		if dir.current_is_dir():
			...
			Debug.post("Attempting to load spawn list definition: %s from %s." % [definition_id, full_path], "SpawningSystem")
			...
			if file:
				...
				if json.parse(text) == OK:
					...
					if data is Dictionary:
						...
						Debug.post("Successfully loaded spawn list: %s" % definition_id, "SpawningSystem")
					else:
...
func execute_spawn_list(list_id: String, base_position: Vector3 = Vector3.ZERO) -> Array[String]:
	Debug.post("execute_spawn_list called for list '%s' at base position %s." % [list_id, base_position], "SpawningSystem")
	...
	Debug.post("[SPAWNER] Executing list: '%s' at position %s" % [list_id, str(base_position)], "SpawningSystem")
	Debug.post("Found %d spawn entries in list '%s'." % [spawns.size(), list_id], "SpawningSystem")
	...
	for spawn_info in spawns:
		...
		Debug.post("[SPAWNER] -- Requesting entity '%s' at offset %s" % [def_id, str(relative_position)], "SpawningSystem")
		...
		Debug.post("Requested new entity '%s', received instance ID '%s'." % [def_id, new_id], "SpawningSystem")

	Debug.post("[SPAWNER] List execution finished.", "SpawningSystem")
	Debug.post("execute_spawn_list finished for list '%s'. Spawned IDs: %s" % [list_id, spawned_instance_ids], "SpawningSystem")
